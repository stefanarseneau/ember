...
def _make_xpphotometry(source_ids : list[str], systems : list[str], username : str = None, password : str = None) -> pd.DataFrame:
    ...
    original, sampling = calibrate(source_ids, save_file=False, truncation=False, username=username, password=password)
...
def process_dataframe(
    ...
    extinction_vec = util.fetch_extinction(np.array([ff[sys].lambda_eff for sys in phot_system]))
    # preprocess the dataframe
    df = util.check_valid(df, use_gravz)
    # fetch xp photometry
    source_ids = df['gaia_dr3_source_id'].values.astype(str).tolist()
    synphot, _, _ = _make_xpphotometry(source_ids, systems=phot_system, username=username, password=password)
    ...
    return pd.merge(df, synphot, on='gaia_dr3_source_id'), flux_dict, extinction_vec
